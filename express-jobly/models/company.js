...
class Company {
   ...
   * Returns [{ handle, name, description, numEmployees, logoUrl }, ...]
   * 
   * If filters of name, min-employees, max-employees are passed in to the function, adjusts the query accordingly by concatenating strings to the database query. By default, the query string filters are set to an empty Object.
   * */
  ...
  static async findAll(queryStringFilters = {}) {
    ...
    const { minEmployees, maxEmployees, name } = queryStringFilters;
    ...
    if (minEmployees) {
      ...
      filterStatements.push(`num_employees >= $${filterValues.length}`);
    }
    if (maxEmployees) {
    ...
    if (name) {
      ...
      filterStatements.push(`name ILIKE $${filterValues.length}`);
    }
