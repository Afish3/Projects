...
def users_show(user_id):
    ...
    session['RECENT_URL'] = f'/users/{user_id}'
...
def add_like(msg_id):
    ...
    return redirect(session['RECENT_URL'])
...
def homepage():
    ...
    if g.user:
        messages = (Message
                    .query
                    .filter(or_(Message.user_id == g.user.id, Message.user_id.in_(user.id for user in g.user.following)))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
